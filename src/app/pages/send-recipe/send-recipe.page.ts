...
@Component({
  ...
  templateUrl: './send-recipe.page.html',
  styleUrls: ['./send-recipe.page.scss'],
})
export class SendRecipePage implements OnInit {
	detailRecipe:any = null;
	fileUrl:any      = null;
	fileUrl2:any     = 1;
	imageURI:any     = null;
	userDetail:any;
    ...
    async presentActionSheet() {
  	...
  	sendRecipe(form: NgForm){
  		if (!this.fileUrl) {
            ...
            let det = JSON.parse(localStorage.getItem('activeDetailRecipe'));
	    	this.authService.sendRecipe(form.value.detail , det.id).subscribe(
	      		data => {
	               	console.log(data);
	               	this.loadingController.dismiss();
	               	if (data['success'] != true) {
	               		this.presentToast('Error al enviar la receta, intente mas tarde');
	               	}else{
	               		this.presentAlert(data['points']);
	               		localStorage.setItem('recipeIns' , 'correcto');
                        ...
                        let dv = document.getElementById('pyro2');
    ...
    uploadFile(){
        ...
        this.recipeDetail = JSON.parse(localStorage.getItem('activeRecipe'));

        fileTransfer.upload(this.imageURI, this.env.API_URL+'/api/uploadPhoto/recipe/'+this.recipeDetail.id+'/'+this.userDetail.id, options)
        .then((data) => {
            var dd = JSON.parse(data['response']);
            localStorage.setItem('activeDetailRecipe' , dd.file);
            console.log(dd);
    ...
    updatePhoto(file){
        let preview = document.getElementById('divAddPhoto');
        let det = JSON.parse(localStorage.getItem('activeDetailRecipe'));
        if (preview) {
